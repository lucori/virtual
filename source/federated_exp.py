...
from data_utils import permuted_mnist, femnist, mnist
...
def run(session, data_set, training, model):
    get_globals()
    config = gpu_session(num_gpus=session['num_gpus'], gpus=session['gpus'])
    if config:
    ...
    ex.info['virtual_history'] = history_virtual
    ex.info['virtual_evaluation'] = evaluation_virtual
    ex.info['local_history'] = history_local
    ex.info['local_evaluation'] = evaluation_local
    ex.info['global_history'] = history_global
    ex.info['global_evaluation'] = evaluation_global
    ex.info['local_average'] = local_average
    ex.info['virtual_average'] = virtual_average
    ex.info['aulc'] = aulcs
    ex.info['normalized_virtual_accuracy'] = normalized_accuracy

    print('done')
    return evaluation_virtual, evaluation_local, evaluation_global
