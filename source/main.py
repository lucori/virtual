...
import datetime
from utils import KLWeightingScheduler
...
HP_KL_WEIGHTING = hp.HParam('kl_weighting', hp.Discrete([1/1e6, 1/1e5, 1/1e4]))
HP_BATCH_SIZE = hp.HParam('batch_size', hp.Discrete([1, 20, inf]))
HP_EPOCHS_PER_ROUND = hp.HParam('epochs_per_round', hp.Discrete([1, 5, 10, 20]))
...
with tf.summary.create_file_writer(logdir).as_default():
    hp.hparams_config(hparams=[HP_KL_WEIGHTING, HP_BATCH_SIZE, HP_EPOCHS_PER_ROUND, HP_OPTIMIZER],
    ...
    for batch_size in HP_BATCH_SIZE.domain.values:
        for epochs_per_round in HP_EPOCHS_PER_ROUND.domain.values:
            for optimizer in HP_OPTIMIZER.domain.values:
                ...
                run_simulation(model_fn, federated_train_data, federated_test_data, model_conf, training_conf,
                               logdir_run)
                session_num += 1
